...
import { HomeBoard, HomeLayoutReset } from "@/components/home/home-board";
import { HomeLayoutProvider } from "@/components/home/use-home-layout";
...
export type HomeAlert = { id: string; title: string; body: string };
export type HomeRecentDeal = {
  id: string;
  title: string;
...
export function OwnerDesk({
  ...
  return (
    <HomeLayoutProvider scope={scope}>
      <div className="space-y-4">
        <section className="ff-card overflow-hidden">
          <div className="flex flex-col gap-3 border-b border-border bg-[color:var(--ff-wash)] px-4 py-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="shrink-0 rounded-md border border-border bg-card px-3 py-2 text-base text-muted-foreground">
              <div className="font-medium text-navy">{scope.label}</div>
              <div>
                {scope.role === "agent" ? "Agent book" : "Admin / owner"} · desk clock {asOf}
              </div>
              <div className="mt-1 text-[11px] text-muted-foreground">
                Grip to move · size 1×1–2×2 · <HomeLayoutReset />
              </div>
            </div>
          </div>
        </section>
          ...
          widgets={{
            ...
            "kpi-written": (
              <KpiLink
                ...
                extra={
                  <span className={writtenTone}>
                    {writtenDelta(snapshot.written.thisMonth.premium, snapshot.written.lastMonth.premium)}
                  </span>
                }
              />
            ),
            ...
            renewals: <RenewalsWidget snapshot={snapshot} />,
            attention: <AttentionWidget items={snapshot.attention} />,
            "line-mix": (
              <MixWidget
                title="Line mix"
                href="/policies?status=in_force"
                blurb="Home, Auto, Flood, Commercial, Health, Life — in-force premium. Empty lines stay at zero."
                slices={snapshot.lineMix}
                empty="No in-force lines yet. Bind a policy or load the owner-book seed."
              />
            ),
            "carrier-mix": (
              <MixWidget
                title="Carrier mix"
                href="/policies?status=in_force"
                blurb="Writing companies on Active / Bound terms. Not a production goal chart."
                slices={snapshot.carrierMix}
                empty="No in-force carriers yet."
              />
            ),
            "cross-sell": <CrossSellWidget snapshot={snapshot} tables={tables} />,
            ana: <AnaWidget />,
            alerts: <AlertsWidget alerts={alerts} />,
            "recent-deals": <RecentDealsWidget deals={recentDeals} />,
          }}
        />
      </div>
    </HomeLayoutProvider>
...
function KpiLink({
  ...
  return (
    <Link href={href} className="group block h-full px-4 pb-4 hover:bg-secondary/40">
      ...
      <div className="mt-0.5 text-base text-muted-foreground">{hint}</div>
      {extra ? <div className="mt-1 text-base">{extra}</div> : null}
    </Link>
  );
}

function RenewalsWidget({ snapshot }: { snapshot: OwnerHomeSnapshot }) {
  return (
    <div className="px-4 pb-4">
      ...
      <div className="mt-3 grid grid-cols-2 gap-3">
        <Link href="/policies?renewal=30" className="rounded-md border border-border bg-secondary/70 px-3 py-3 hover:border-primary">
          ...
          <div className="text-base text-muted-foreground">{formatMoney(snapshot.renewals30.premium)}</div>
        </Link>
        <Link href="/policies?renewal=60" className="rounded-md border border-border bg-secondary/70 px-3 py-3 hover:border-primary">
          ...
          <div className="text-base text-muted-foreground">{formatMoney(snapshot.renewals60.premium)}</div>
        </Link>
      </div>
...
function AttentionWidget({ items }: { items: OwnerHomeSnapshot["attention"] }) {
  return (
    <div>
      <div className="flex items-center justify-between border-b border-border px-4 pb-3">
        <div>
          <h3 className="text-base font-semibold text-navy">Needs attention</h3>
      ...
      {items.length === 0 ? (
        <p className="px-4 py-6 text-base text-muted-foreground">Nothing flagged on the book.</p>
      ) : (
            ...
            <li key={item.id}>
              <Link href={item.href} className="flex items-start gap-3 px-4 py-3 hover:bg-secondary/50">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-base text-muted-foreground">{item.detail}</div>
                </div>
              </Link>
            </li>
          ))}
...
function MixWidget({
  ...
  href: string;
  blurb: string;
  slices: OwnerHomeSnapshot["lineMix"];
  empty: string;
}) {
  return (
    <div className="px-4 pb-4">
      <Header title={title} href={href} action="Policies" />
      <p className="mb-3 mt-1 text-base text-muted-foreground">{blurb}</p>
      <MixBars slices={slices} empty={empty} />
    </div>
  );
}
    ...
    <div>
      <div className="flex items-center justify-between border-b border-border px-4 pb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-base font-semibold text-navy">Cross-sell gaps</h3>
            <p className="text-base text-muted-foreground">
        ...
        <ul className="divide-y divide-border">
          {snapshot.gaps.map((gap) => (
            <li key={gap.contactId} className="flex items-center justify-between gap-3 px-4 py-2.5 text-sm">
              ...
              <span className="text-base text-muted-foreground">Needs {gap.missing.join(", ")}</span>
            </li>
          ))}
        </ul>
      )}
...
function AnaWidget() {
  return (
    <div className="px-4 pb-4">
      <h3 className="text-base font-semibold text-navy">Ana Dib shop</h3>
...
function AlertsWidget({ alerts }: { alerts: HomeAlert[] }) {
  return (
    <div>
      <div className="border-b border-border px-4 pb-2 text-base font-semibold text-navy">
        In-app alerts
      </div>
      {alerts.length === 0 ? (
...
function RecentDealsWidget({ deals }: { deals: HomeRecentDeal[] }) {
  return (
    <div>
      <div className="border-b border-border px-4 pb-2 text-base font-semibold text-navy">
        Recent deals

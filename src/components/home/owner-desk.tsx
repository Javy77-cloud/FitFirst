...
import { CompactMix } from "./mix-bars";
    ...
    <div className="space-y-4">
      <section className="ff-card overflow-hidden">
        <div className="flex flex-col gap-3 border-b border-border bg-[color:var(--ff-wash)] px-4 py-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="shrink-0 rounded-md border border-border bg-card px-3 py-2 text-sm text-muted-foreground">
        ...
        <section className="ff-card p-4">
          ...
          <div className="mt-3 grid grid-cols-2 gap-3">
            <Link href="/policies?renewal=30" className="rounded-md border border-border bg-secondary/70 px-3 py-3 hover:border-primary">
              <div className="text-xs uppercase tracking-wide text-muted-foreground">In 30 days</div>
              <div className="text-xl font-semibold text-navy">{fmt(snapshot.renewals30.count)}</div>
            ...
            <Link href="/policies?renewal=60" className="rounded-md border border-border bg-secondary/70 px-3 py-3 hover:border-primary">
              <div className="text-xs uppercase tracking-wide text-muted-foreground">In 60 days</div>
              <div className="text-xl font-semibold text-navy">{fmt(snapshot.renewals60.count)}</div>
                ...
                <li key={item.id}>
                  <Link href={item.href} className="flex items-start gap-3 px-4 py-3 hover:bg-secondary/50">
                    ...
                    <div className="min-w-0">
                      <div className="text-base font-medium text-navy">{item.title}</div>
      ...
      <section className="ff-card p-4">
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            ...
            <p className="mb-2 mt-0.5 text-sm text-muted-foreground">In-force premium. Empty lines stay off the bar.</p>
            <CompactMix slices={snapshot.lineMix} empty="No in-force lines yet." />
          </div>
          <div>
            <Header title="Carrier mix" href="/policies?status=in_force" action="Policies" />
            <p className="mb-2 mt-0.5 text-sm text-muted-foreground">Writing companies on Active / Bound terms.</p>
            <CompactMix slices={snapshot.carrierMix} empty="No in-force carriers yet." />
          </div>
        </div>
      </section>

      <div className="grid gap-4 lg:grid-cols-2">
        <CrossSellPanel chips={snapshot.sellableChips} gaps={snapshot.gaps} selected={need} />

        <section className="ff-card p-4">
          <h3 className="text-base font-semibold text-navy">Ana Dib shop</h3>
          <p className="mt-1 text-base text-muted-foreground">
            The 2026-09-02 Palm Bay HO3 is still unbound. Coverage A is $321,000. Eight markets,
            zero bindable. Prior-book HO3 is in force — Auto, Flood, and Umbrella stay open gaps.
          </p>
...
function KpiLink({
  ...
  return (
    <Link href={href} className="ff-card group p-4 hover:border-primary">
      ...
      <div className="truncate text-3xl font-semibold tabular-nums text-navy">{value}</div>

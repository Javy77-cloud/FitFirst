          ...
          <div className="flex flex-col gap-3 border-b border-border bg-[color:var(--ff-wash)] px-4 py-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="flex shrink-0 flex-col items-stretch gap-2 sm:items-end">
              {scope.role !== "agent" && bookOptions.length > 0 ? (
                <BookScopeFilter options={bookOptions} current={bookValue} attention={attentionValue} />
              ) : null}
        ...
        <HomeBoard
          widgets={{
                  ...
                  "kpi-written": (
                    <KpiLink
                      ...
                      extra={
                        <span
                          className={momTone(
                            snapshot.written.thisMonth.premium,
                            snapshot.written.lastMonth.premium,
                          )}
                        >
            ...
            ...(tile("strip")
              ? {
                  strip: (
                    <div className="grid h-full grid-cols-2 gap-2 p-3 sm:grid-cols-3">
                      ...
                      <StripLink
                        href="/deals?stage=quote_sent"
                        label="Quote sent"
                        value={fmt(snapshot.pipeline.quoteSent)}
                      />
              ...
              : {}),
            ...(isAdmin && tile("hit-lost") && hitLost
              ? { "hit-lost": <HitLostCards report={hitLost} embedded /> }
              : {}),
            ...(tile("company")
              ? {
                  company: (
...
function KpiLink({
  ...
  return (
    <Link
      href={href}
      className={
        framed ? "ff-card group p-3 hover:border-primary" : "group flex h-full flex-col p-3 hover:bg-secondary/40"
      }
    >

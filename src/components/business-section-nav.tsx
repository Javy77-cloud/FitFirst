...
export type BusinessSectionCounts = Partial<Record<BusinessSectionId, number>>;
...
export function BusinessSectionNav({
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (id) setActive(id as BusinessSectionId);
      },
  ...
  function onSave() {
    startTransition(async () => {
      try {
        await saveBusinessSectionNav(draftSelected);
  ...
  function onReset() {
    startTransition(async () => {
      try {
        const saved = await resetBusinessSectionNav();
  ...
  return (
    <div ref={hostRef} className="relative mb-3" data-ff-business-nav-host-box="">
      <div ref={sentinelRef} className="h-0 w-full" aria-hidden data-ff-business-nav-sentinel="" />
      {pinned ? <div style={{ height: navHeight }} aria-hidden data-ff-business-nav-spacer="" /> : null}
      <nav
        ref={navRef}
        ...
        data-ff-business-section-nav="top"
        data-ff-business-nav-pinned={pinned ? "1" : "0"}
      >
        <div
          ref={chipsRef}
          className={cn(
      ...
      <Dialog open={customizeOpen} onOpenChange={setCustomizeOpen}>
        <DialogContent className="sm:max-w-lg" data-ff-business-nav-customize="">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-md border border-border p-2" data-ff-business-nav-selected="">
              <p className="mb-2 px-1 text-xs font-semibold text-muted-foreground">
                Selected ({draftSelected.length}/{BUSINESS_SECTION_NAV_MAX})
              </p>
            ...
            <div className="rounded-md border border-border p-2" data-ff-business-nav-available="">
              <p className="mb-2 px-1 text-xs font-semibold text-muted-foreground">
                Available
              </p>
              <ul className="space-y-1">
                {available.map((section) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      disabled={draftSelected.length >= BUSINESS_SECTION_NAV_MAX}
          ...
          <p className="text-[11px] text-muted-foreground">
            Default:{" "}
            {DEFAULT_BUSINESS_SECTION_NAV_IDS.map(
              (id) => BUSINESS_SECTION_POOL.find((s) => s.id === id)?.label,
            ).join(", ")}
            .
          </p>
        </DialogContent>

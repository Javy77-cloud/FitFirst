...
import {
  applyBusinessLayoutTemplate,
  readBusinessLayoutTemplate,
} from "@/app/actions/businesses-ops";
import { Button } from "@/components/ui/button";
import type { LayoutTemplateKind } from "@/lib/custom-fields/layout-template";

/** Classic (Dense, one column) + Card (Two column) — persist per agency via businesses layout. */
export function BusinessLayoutTemplatePicker({
  accountId,
  onApplied,
}: {
  accountId: string;
  ...
  useEffect(() => {
    let cancelled = false;
    void readBusinessLayoutTemplate().then((kind) => {
  ...
  async function apply(kind: "card" | "classic") {
    ...
    try {
      ...
      await applyBusinessLayoutTemplate(fd);
      setActive(kind);
      onApplied?.();
      router.refresh();
      void accountId;
  ...
  return (
    <div className="flex flex-wrap gap-2" data-ff-business-layout-templates="">
      <Button
        type="button"
        size="sm"

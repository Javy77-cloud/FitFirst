...
export function QuotesPanel({
  ...
  const proposals = quoteDocs.filter((doc) => isProposalDoc(doc) || isProposalAttachment(doc));
  ...
  return (
    <div className="space-y-4">
      ...
      {quoteDocs.filter((d) => !isProposalDoc(d) && !isProposalAttachment(d)).length > 0 ? (
        <p className="text-xs text-muted-foreground">
          {quoteDocs.filter((d) => !isProposalDoc(d) && !isProposalAttachment(d)).length} quote PDF
          {quoteDocs.filter((d) => !isProposalDoc(d) && !isProposalAttachment(d)).length === 1 ? "" : "s"}{" "}
          attached on this deal. These files are not policies.
        </p>
      ) : null}

...
describe("Deals page sep7h", () => {
  ...
  it("keeps attach-documents on the left band, ~120px, beside Today's Activity", () => {
    ...
    expect(page).not.toMatch(/flex-1/);
    ...
    expect(strip).toMatch(/deal-today-item inline-flex shrink-0 flex-col/);
    expect(strip).toMatch(/deal-today-chip/);
    expect(strip).toMatch(/deal-today-chip-label/);
    ...
    expect(chrome).toMatch(/width: 60px/);
    expect(chrome).toMatch(/height: 60px/);
    expect(chrome).toMatch(/max-height: 60px/);

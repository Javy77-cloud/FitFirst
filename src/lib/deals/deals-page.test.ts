...
describe("Deals page sep7h", () => {
  ...
  it("keeps attach-documents on the left band, ~120px, beside Today's Activity", () => {
    ...
    expect(page).toMatch(/w-\[min\(38rem,60%\)\]/);
    ...
    expect(page).not.toMatch(/w-full/);
    ...
    expect(chrome).toMatch(/width: min\(38rem, 60%\)/);
    ...
    expect(chrome).toMatch(/min-height: 120px/);

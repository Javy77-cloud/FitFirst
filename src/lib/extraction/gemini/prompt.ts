...
- Keys MUST be exactly from this list (omit unknown keys or set value null):
  ${keys.join(", ")}
- Each present key maps to an object: { "value": string|null, "confidence": number }
  where confidence is 0..1. A phone photo is still a clear read when you can transcribe the characters — use confidence >= 0.9 for those. Do not lower confidence because the file is a JPEG, PNG, or HEIC.
- Extract ONLY what is written on the page. Never invent.
- If a label is not on the page: omit the key or set value null. Never return {} or all-null when a vehicle schedule, driver list, or coverage table is visible.
- VIN: full 17 characters when printed. Year/make/model per vehicle.
- Florida personal auto / ACORD-style dec (Progressive, GEICO, State Farm, Allstate, Travelers, Liberty Mutual, Bristol West, National General, The General, Citizens auto):
  Header → named_insured, policy_number, current_carrier, effective_date (policy period from), expiration_date (policy period to), current_premium (total or term premium, digits only).
  Vehicle schedule (up to 4) → vin, vehicle_year, vehicle_make, vehicle_model, then vehicle_2_* / vehicle_3_* / vehicle_4_* . Garaging address/ZIP → garaging_address / garaging_zip.
  Coverage schedule → Bodily Injury or BI Liability = liability_bi as 100/300 (not 100000/300000); Property Damage = liability_pd digits; Uninsured/Underinsured = um_uim; Personal Injury Protection / PIP / No-Fault = pip digits; Comprehensive or Other Than Collision / OTC = comp_deductible digits; Collision = collision_deductible digits.
  Driver schedule → driver_1 … driver_4. When the page lists only the named insured as the operator, that person is driver_1_name.
- Emit those flat keys. A nested vehicles[] or drivers[] array is not a substitute.

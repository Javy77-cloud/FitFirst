...
1. Vehicles and VIN. vin is the 17-character vehicle identification number (labels: VIN, V.I.N., Vehicle Identification No). vehicle_year, vehicle_make, vehicle_model from columns or from one cell such as "2019 TOYOTA CAMRY". More vehicles: vehicle_2_*, vehicle_3_*, vehicle_4_* (vin, year, make, model).
2. Drivers. driver_1_name, driver_1_dob, driver_1_license, then driver_2_name, driver_2_dob, driver_2_license. driver_1_name is the full printed legal name: first, middle name or middle initial, and the complete last name. Do not truncate. "Domenic M Iori" stays "Domenic M Iori" — never "Domenic Ic", "Domenic I", or "D. Iori". If First / Middle / Last are separate columns, join them in that order. Add gender (Male/Female only), marital status, and relationship for drivers 2+ when printed. Never fill driver_1_relationship. Never fill employment — use industry and occupation only when printed.
3. Coverage limits from the coverage table. Bodily Injury or Liability Bodily Injury → liability_bi as 100/300 (not 100000/300000). If Each Person and Each Accident are separate cells, join them as 100/300. Property Damage → liability_pd digits (100000). Uninsured or Underinsured Motorist, including Uninsured Motorist Bodily Injury → um_uim. PIP or Personal Injury Protection → pip digits. Comprehensive or Other Than Collision → comp_deductible (comprehensive deductible). Collision → collision_deductible. Skip a coverage that is not printed.
4. Dates. Policy period from/to → effective_date and expiration_date, as printed.
5. Current Policy, the last Auto risk-profile section. Do not leave it empty when the dec prints insurance details. Copy only what is printed.
   - current_carrier: the named insurer. Labels include Insurance Company, Insurance Name, Named Insurer, Writing Company, Insurer. Copy the full company name.
   - policy_number: Policy Number, Policy No, Policy #, Pol #, or Current policy ID. Copy the whole ID.
   - current_premium: total premium for the term (Total Premium / Policy Premium / 6-month premium), digits only, when a premium is printed.
   - years_with_carrier: only a printed number of years. Do not calculate it from the dates.
   - currently_insured: only when the page states prior or continuous coverage. Use "Currently insured 6 months or more" only when 6 months or more is printed. A declarations page alone does not mean that.
   - aaa_member: only when AAA membership or None is printed. "None", "No", or not a member → None. Printed 1–9 years → "1–9 years". Printed 10 or more years → "10+ years". Do not guess a tenure.

Example (include only keys you can read):
{"vin":{"value":"4T1B11HK5KU123456","confidence":0.95},"vehicle_year":{"value":"2019","confidence":0.95},"vehicle_make":{"value":"TOYOTA","confidence":0.95},"vehicle_model":{"value":"CAMRY","confidence":0.9},"driver_1_name":{"value":"Domenic M Iori","confidence":0.95},"driver_1_dob":{"value":"04/02/1984","confidence":0.9},"driver_1_license":{"value":"R400-123-45-678","confidence":0.9},"liability_bi":{"value":"100/300","confidence":0.9},"liability_pd":{"value":"100000","confidence":0.9},"um_uim":{"value":"100/300","confidence":0.9},"pip":{"value":"10000","confidence":0.9},"comp_deductible":{"value":"500","confidence":0.9},"collision_deductible":{"value":"500","confidence":0.9},"effective_date":{"value":"03/15/2026","confidence":0.9},"expiration_date":{"value":"09/15/2026","confidence":0.9},"current_carrier":{"value":"Progressive American Insurance Company","confidence":0.95},"policy_number":{"value":"923456789","confidence":0.95},"current_premium":{"value":"1640","confidence":0.9}}

Current Policy example (filled only because each fact is printed on that sample page). Never copy these sample values onto a page that does not show them. Omit years_with_carrier, currently_insured, and aaa_member when the page does not state them:
{"current_carrier":{"value":"Progressive American Insurance Company","confidence":0.95},"policy_number":{"value":"923456789","confidence":0.95},"effective_date":{"value":"03/15/2026","confidence":0.95},"expiration_date":{"value":"09/15/2026","confidence":0.95},"current_premium":{"value":"1640","confidence":0.9},"years_with_carrier":{"value":"3","confidence":0.9},"currently_insured":{"value":"Currently insured 6 months or more","confidence":0.9},"aaa_member":{"value":"None","confidence":0.9}}

Other allowed keys when printed: ${keys.join(", ")}
Money: digits only (no $). Dates: keep as printed. Never invent a carrier, policy ID, premium, date, coverage, VIN, or driver name.
`;
  }
...
export function buildGeminiUserPrompt(docType?: string | null, shopLine?: string | null): string {
  ...
  if (line === "auto" || line === "motorcycle" || line === "commercial_auto") {
    focus =
      "This photo or PDF is an Auto policy or Auto declaration (carrier dec, policy jacket, ID card, or ACORD 90). Do not treat this as homeowners / Coverage A. Read the page and fill, in order: vin and vehicle_year / vehicle_make / vehicle_model (split a cell like 2019 TOYOTA CAMRY; vehicle_2_* for the next car); driver_1_name is the full legal name including middle initial — Domenic M Iori, never Domenic Ic — plus driver_1_dob, driver_1_license and driver_2_* when a second driver is listed; coverage limits from the coverage table — Bodily Injury liability_bi as 100/300, Property Damage liability_pd as digits, UM/UIM um_uim, PIP pip, comprehensive deductible comp_deductible, collision_deductible; effective_date and expiration_date from the policy period; Current Policy: current_carrier (named insurer / insurance company, full name), policy_number (Policy # / Current policy ID), current_premium when a premium is printed. years_with_carrier, currently_insured, and aaa_member only when those facts are printed — do not invent them. Also fill when printed: industry, occupation, gender (Male/Female only), marital status, garaging address and ZIP. Never fill driver_1_relationship. Never fill employment / employment status — use industry + occupation only. Do not return {} and do not mark the page not_declaration when a VIN, vehicle, driver, coverage table, carrier, or policy number is visible.";
  }

...
- Alarm certificate (ADT, central station, fire alarm, or burglar alarm certificate): when a monitored alarm is in service, set fire_alarm to yes and central_alarm to yes. Leave both null when the page does not show an alarm in service. Do not invent a policy number, premium, or coverages from the certificate.
`;
}
...
export function buildGeminiUserPrompt(docType?: string | null, shopLine?: string | null): string {
  ...
  if (kind === "wind_mit" || kind.includes("wind")) {
    ...
    kind === "alarm_certificate" ||
    kind === "alarm" ||
    kind === "certificate" ||
    kind.includes("alarm")
  ) {
    focus =
      "This is a central-station alarm certificate (ADT or similar), not a homeowners dec and not a flood policy. When the page shows a monitored, central, fire, or burglar alarm in service, set fire_alarm to yes and central_alarm to yes. If no alarm is shown in service, leave both null — never invent yes. Fill applicant_name and property_address when printed. Do not invent policy number, premium, coverages, or wind-mitigation fields.";

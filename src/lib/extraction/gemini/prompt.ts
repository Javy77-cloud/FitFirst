...
- Dec: Named insured; Residence premises / Location description / Property information / insured property → property_address
  ...
  DP-3 / dwelling fire (Tower Hill and the same schedule): read every page. Rating Information may sit on a page other than Basic Policy Coverages — still fill year_built, construction_type, occupancy, occupied_by, usage_type, protection_class, bceg_grade, number_of_families, sprinkler, fire_alarm, roof_year, roof_material, roof_shape, and opening_protection / hurricane_protection when printed anywhere. Do not stop after the coverage table.
  Coverage L - Liability (Each Occurrence) is coverage_l + coverage_l_premium, not coverage_e. Coverage M - Medical Payments to Others (Each Person) is coverage_m + coverage_m_premium, not coverage_f. On DP-3, Coverage D Fair Rental Value stays coverage_d, and Coverage E Additional Living Expense stays coverage_e.
  Limited Fungi … Liability is limited_fungi_liability (for example 50000) + limited_fungi_liability_premium. Limited Fungi … Property stays limited_fungi ($10,000/$10,000). Do not merge them.
  Rental to Others (Short Term Exclusions) - Property → rental_to_others_short_term, premium Included when the premium column is Incl and the limit is blank. Replacement Cost Buy Back → replacement_cost_buy_back the same way.
  Sinkhole Exclusion Included means the exclusion is on the policy → sinkhole_exclusion Included and sinkhole_deductible Not Included. Water Damage Exclusion Included → water_damage_exclusion Included (the desk shows Water Damage Not Covered).
  When Coverage A prints Fire, Extended Coverage, and Hurricane premium columns plus an Annual Premium, coverage_a_premium is that Annual Premium (Fire + Extended + Hurricane, such as 2738). Also return coverage_a_fire_premium, coverage_a_extended_premium, and coverage_a_hurricane_premium when those columns are printed. coverage_a_premium is never the Total Policy Premium.
  form stays DP-3 or DP3, never Home. Do not invent desk rows for Age of Dwelling Surcharge, EMPAT, FIGA, MGA Fee, Surplus Contribution, Advance Quote Discount, Age of Roof Credit, Deductible Options, or Decreased Coverage B Limit.

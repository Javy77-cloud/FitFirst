...
- Each present key maps to an object: { "value": string|null, "confidence": number }
  where confidence is 0..1 (1 = clearly printed on the page).
- Extract ONLY what is written on the page. Never invent.
- If unknown or not present: omit the key. Do not invent a value.
...
- Phone photos and PDFs of auto declarations, ID cards, and ACORD 90 are in scope. Omit keys that are not printed. Never return an empty object when a VIN, vehicle, driver, premium, or coverage is visible.
- Vehicle schedule: Year / Make / Model / VIN columns, or one cell such as "2019 TOYOTA CAMRY" plus a VIN. Split that cell into vehicle_year, vehicle_make, vehicle_model, and vin. Further vehicles use vehicle_2_*, vehicle_3_*, vehicle_4_*.
- Coverage table: Bodily Injury → liability_bi (100/300); Property Damage → liability_pd (digits); Uninsured or Underinsured Motorist → um_uim; PIP or Personal Injury Protection → pip (digits); Comprehensive or Other Than Collision → comp_deductible; Collision → collision_deductible.
- Policy period from/to → effective_date and expiration_date. Total premium or 6-month premium → current_premium (digits only).
- Prefer these flat keys. If you also group vehicles, drivers, or coverages, still fill the flat keys.
`;
  }

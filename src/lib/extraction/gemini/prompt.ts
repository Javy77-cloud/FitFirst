...
- Dec: Named insured; Residence premises / Location / property_address; Coverage A / dwelling; hurricane / AOP / wind-hail deductibles;
  policy number (Policy No / Pol # / Policy # → policy_number); premium / current_premium / total premium / annual premium
  (issued total — not a prior quote); effective/expiration/renewal_date (Eff date / policy period start → effective_date);
  selling_agency; producer; insurance_type; form (HO3/DP-3); billing_frequency; next_due; payment_method;
  mortgagee + mortgagee_address; loan number; roof_year / roof_age; ordinance or law; water backup; scheduled personal property;
...
export function buildGeminiUserPrompt(docType?: string | null, shopLine?: string | null): string {
  ...
  if (kind === "wind_mit" || kind.includes("wind")) {
    ...
    focus =
      "This is a dec/policy. MUST fill when present: named_insured/current_policy_name_insured, secondary_named_insured, property_address, coverage_a (dwelling), coverage_b, coverage_c, coverage_d, coverage_e, coverage_f, hurricane_deductible, aop_deductible, wind_hail_deductible, ordinance_law, water_backup, sinkhole_deductible, policy_number (Florida Peninsula and similar: Policy No / Pol # / Policy # / policy number all map to policy_number — never leave it empty when printed), premium, current_premium (same issued total premium on a dec — Total Premium / Annual Premium / Total Annual Premium; not a shopping quote), current_carrier, effective_date (Eff date / policy period start), expiration_date, renewal_date, selling_agency, producer, insurance_type, billing_frequency, next_due, payment_method, mortgagee, mortgagee_address, loan_number, protection_class, number_of_families, year_built, construction_type, occupancy, usage, form (HO3/DP-3/DP3/HO-3 etc from title or Form line), opening_protection, sprinkler (Automatic Sprinklers), fire_alarm / central_alarm (Fire Alarm / Burglar), bceg_grade (BCEG Grade), roof_covering, roof_shape, roof_year, roof_age, loss_of_rents / fair_rental_value (Coverage D Fair Rental Value limit), landlord_liability (Coverage L Personal Liability when dwelling/DP). For sinkhole_deductible: if the dec says the policy does NOT provide sinkhole coverage (or only catastrophic ground cover collapse) and sinkhole may be purchased for additional premium, set sinkhole_deductible to No — do not leave it blank. For water_backup: only fill when an endorsement/limit is printed; do not invent. For sprinkler/fire_alarm/opening_protection: when Rating Information says None, set value to no or None — do not leave blank. Cov A alone is OK when B–F are missing. If the image is actually a four-point (or clearly shows Date Inspected / Date of Inspection), ALSO fill date_inspected and four_point_date from that top inspection date — do not ignore it just because the upload type said dec. Also set document_kind to declaration if this is a declarations page. If this is a quote packet, wind mit, inspection, or otherwise does not look like a dec, set document_kind to not_declaration and leave policy fields empty.";

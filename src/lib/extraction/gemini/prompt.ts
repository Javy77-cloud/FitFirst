...
- Money: premiums are digits only (no $); keep cents, including a printed negative premium such as -82.40. Coverage A, B, C, D, E, and F dollar limits use $130,000 (a printed percent stays 10%). Hurricane, all-other-perils (AOP), and wind/hail deductibles that are dollar amounts use $1,000; a percent stays 2% or 2% of Coverage A. Optional-coverage dollar limits (personal injury, home computer, ordinance or law, water backup) use $100,000. Included stays Included. Dates: keep as printed. Stories / year_built: integers as strings.
...
- Dec: Named insured; Residence premises / Location description / Property information / insured property → property_address
  ...
  Coverage A / dwelling; Section I deductibles kept separate — All Other Perils → aop_deductible,
  Windstorm or Hail (Other Than Hurricane) → wind_hail_deductible, Hurricane → hurricane_deductible
  (store 2% or 2% of Coverage A, not only the parenthetical dollar), Sinkhole Not Included → sinkhole_deductible Not Included or None;
  ...
  ordinance or law (limit + ordinance_law_premium, keep a negative premium); water backup / Water Back Up and Sump Overflow (limit + water_backup_premium);
  personal injury (personal_injury + personal_injury_premium; Included stays Included);
  personal property replacement cost (Included or Y/N, plus personal_property_replacement_cost_premium);
  ...
  Rating Information, only when that label has a printed value (a blank rating cell is None — do not invent Masonry, a year, or Owner):
  construction_type (keep Masonry on construction_type; do not store it as occupancy); year_built (printed Year of Construction — the key stays year_built); roof_year (Year of Roof/Updated);
  type_of_residence (Owner Occupied); dwelling_type (Single Family); months_occupied (9 to 12 Months); occupancy (the Occupancy line: Owner or Tenant). When the Occupancy line is missing, also set occupancy from Type of Residence (Owner Occupied → occupancy Owner, Tenant Occupied → occupancy Tenant) and still store the printed type_of_residence.

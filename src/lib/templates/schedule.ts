...
export async function scheduleWonClientEmails(input: WonScheduleInput): Promise<string[]> {
  ...
  for (const trigger of triggers) {
    if (!trigger.enabled || !trigger.emailClient || !trigger.templateId) continue;
    ...
    const scheduledFor = addDelay(
      input.wonAt,
      trigger.delayAmount ?? 0,
      (trigger.delayUnit ?? "days") as EmailDelayUnit,
    );
...
export async function schedulePolicyRenewalEmails(
  ...
  for (const trigger of triggers) {
    ...
    if (!trigger.emailClient || !trigger.templateId) continue;
    const [template] = await db
      .select()
      .from(emailTemplates)
      .where(eq(emailTemplates.id, trigger.templateId));

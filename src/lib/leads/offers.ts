...
import { alerts, leadOffers, leads, users } from "@/lib/db/schema";
...
async function upsertOffer(input: {
  ...
  const [existing] = await db
    .select()
    .from(leadOffers)
    .where(and(eq(leadOffers.tenantId, DEFAULT_TENANT_ID), eq(leadOffers.leadId, input.leadId)));
  const now = new Date();
  const patch = {
    source: input.source,
  ...
  if (existing) {
    const [row] = await db
      .update(leadOffers)
      .set(patch)
      .where(eq(leadOffers.id, existing.id))
      .returning();
    return row;
  }
  const [row] = await db
...
export async function listOpenLeadOffers(): Promise<LeadOfferBulletinRow[]> {
  const rows = await db
    ...
    .where(and(eq(leadOffers.tenantId, DEFAULT_TENANT_ID), eq(leadOffers.status, "open")))

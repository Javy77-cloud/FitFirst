...
describe("deal Documents save must not open error.tsx", () => {
  ...
  it("soft-fails the Documents panel instead of the whole deal page", () => {
    ...
    expect(source("src/components/deal/documents-panel.tsx")).not.toMatch(/dealId \}/);
    expect(source("src/app/actions/documents.ts")).not.toMatch(/linkDealDocumentToProduct/);
    expect(source("src/app/documents/page.tsx")).not.toMatch(/liveDealLibraryDocs/);
    expect(source("src/app/documents/page.tsx")).not.toMatch(/params\.deal/);

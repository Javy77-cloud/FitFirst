...
export async function getPipelineBoard(slug: string, sub?: { lifeSub?: string; healthSub?: string }) {
  ...
  const boards = visiblePipelineBoards(await listPipelines(), lineSettings);
  ...
  return {
    ...
    boards,
    lineSettings,
    cards: cards.filter((deal) => {
      if (deal.pipelineId !== board.id) return false;
      if (board.slug === "life" && sub?.lifeSub && sub.lifeSub !== "all") {
        return matchesLifeOrHealthSub(deal.policySubType, sub.lifeSub, lineSettings.lifeOptions);
      }
      if (board.slug === "health" && sub?.healthSub && sub.healthSub !== "all") {
        return matchesLifeOrHealthSub(deal.policySubType, sub.healthSub, lineSettings.healthOptions);
      }
      return true;
    }),
  };

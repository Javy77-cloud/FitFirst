...
import { AgencyLineCatalog } from "@/components/settings/agency-line-catalog";
...
import { listAgencyLineOrphans, loadAgencyLines } from "@/lib/db/agency-lines";
...
export default async function LinesSettingsPage() {
  const [session, settings, lines, orphans] = await Promise.all([
    requireAdminPage(),
    loadDeskLineSettings(),
    loadAgencyLines(),
    listAgencyLineOrphans(),
  ]);

  return (
    <SettingsShell title="Lines of business" current="lines">
      <AgencyLineCatalog lines={lines} orphans={orphans} canEdit={session.isAdmin} />

      <div className="mt-8 space-y-4">
        <div>
          <h2 className="text-sm font-semibold text-navy">Written books</h2>
          <p className="mt-1 max-w-3xl text-sm text-muted-foreground">
            Hide Life or Health when this agency does not write those books. Nav, pipeline boards,
            and book filters follow these toggles. Selling Agency stays off the day-to-day desk
            unless you turn the picklists on.
          </p>
        </div>
        ...
        <form action={saveWrittenLines} className="ff-list-card max-w-2xl">
            ...
            <fieldset disabled={!session.isAdmin} className="space-y-3">
              <label className="flex items-start gap-2 text-sm">
                <input
                  ...
                  name="writeLife"
                  value="true"
                  defaultChecked={settings.writeLife}
                  className="mt-1"
                />
                <span>

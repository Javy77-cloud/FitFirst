...
export async function recordReconReceived(formData: FormData): Promise<void> {
...
export async function markReconStatus(formData: FormData): Promise<void> {
  ...
  await writeRecon({
    reconId,
    actorId: session.userId ?? "",
    received: nextReceived ?? undefined,
    status,
    note: note || undefined,
  });
}

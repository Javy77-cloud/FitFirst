...
async function assertAdmin(): Promise<{ ok: true; name: string; userId: string | null } | { ok: false; error: string }> {
...
export async function updateCarrierField(input: {
  ...
  if (fieldKey === "status" || fieldKey === "active" || fieldKey === "desk_status") {
    ...
    const t = raw.trim();
    patch.amBestDate = t ? new Date(t) : null;
  } else {
...
export async function archiveCarrier(carrierId: string): Promise<{ ok: true } | { ok: false; error: string }> {
  const admin = await assertAdmin();
  if (!admin.ok) return admin;

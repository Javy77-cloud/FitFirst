...
Agency BYO. Store the key as `FLORIDA_PROPERTY_API_KEY` (already in `.env.example`) or a developer-vault row `provider=florida_property`. No key → toast says so and **nothing is written** — no fake parcel. No `db:seed`. Ana unbound. Cov A **$321,000**. Synonym dictionary from sep7cd is included. Tip SHA `c2634af1` merged with synonym dictionary `331c78d4`.
...
## Previous tip (`cursor/live-ff-tip-sep7cd`)
...
**Yahoo:** [developer.yahoo.com/apps](https://developer.yahoo.com/apps/) → create app, OpenID. Redirect URI must match. Env: `YAHOO_OAUTH_CLIENT_ID` / `YAHOO_OAUTH_CLIENT_SECRET`.

**DocuSign sandbox:** [developers.docusign.com](https://developers.docusign.com/) → Apps and Keys → Integration Key + Secret Key. Redirect URI on the key. Auth base `https://account-d.docusign.com`. Env: `DOCUSIGN_INTEGRATION_KEY` / `DOCUSIGN_SECRET_KEY`.

Run `npm run db:migrate` so `0136_byo_oauth_wave` adds refresh-token columns and `calendar_busy_blocks`.

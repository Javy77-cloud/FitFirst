...
**DIFF WAVE-1 (this branch):** Producer scorecards + Glance (`cursor/producer-scorecards-glance-4f97`, no migration), E&O audit + Admin Compliance (`cursor/eo-audit-compliance-314d`, `0036_eo_audit_logs`), and priority queue + campaign sequences (`cursor/priority-queue-campaigns-1a0d`, incoming `0036` renumbered to `0037_campaign_sequences`). Ana stays unbound at Cov A **$321,000**. Sidebar `#c5ddf4`. One Pipeline. Alerts off the sidebar.

**Wave 2 merged:** Deal quote PDF view / email / SMS / print (`cursor/deal-docs-pdf-view-ba1b`), DOC → master sheet fill (`cursor/doc-master-sheet-fill-1202`, `0034_fill_feedback`), and Fill Learning (`cursor/fill-learning-log-efeb`, incoming `0021` renumbered to `0035_fill_learning_logs`). Next free migration is **0038**. See `COORDINATION.md`.

...
const PORTAL = "https://www.foragentsonly.com/";
const USER = "javy77@";
const NOTE =
  "Auto Progressive — No market: credentials rejected after NordPass autofill — needs NordPass update.";
...
async function main() {
  const rows = await db
    ...
    .where(
      or(
        ilike(carriers.name, "%Progressive%"),
        ilike(carriers.name, "Progressive"),
      ),
    );
  if (!rows.length) throw new Error("Progressive carrier not found");
  // Prefer exact / shortest Progressive name (avoid Bristol West Progressive etc.)
  const carrier =
    rows.find((c) => /^progressive$/i.test(c.name.trim())) ??
    rows.find((c) => /^progressive\b/i.test(c.name.trim())) ??
    rows[0];
    ...
    JSON.stringify(
      {
        carrierId: carrier.id,
        name: carrier.name,
        matchedCandidates: rows.map((r) => ({ id: r.id, name: r.name })),

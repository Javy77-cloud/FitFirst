-- Backfill (WRITES): copy a blank Marketplace / Medicare insured location from
-- the linked contact onto the policy.
--
-- One transaction. Re-running is a no-op once premises are filled.
-- Updates only policies.premises_address, premises_city, premises_state, premises_zip.
-- Does not update renewal_date or any other column, and does not update contacts.
-- Not a drizzle migration — db:migrate will not run this.
--
-- Run the preview and the flag list first.
--
--   psql "$DATABASE_URL" -f scripts/backfill-marketplace-medicare-insured-location.sql
--
-- To see RETURNING without keeping the write, change the final COMMIT to ROLLBACK.
-- Rows updated are only known after this runs on the live database.
...
SET
  premises_address = w.copy_street,
  premises_city = w.copy_city,
  premises_state = w.copy_state,
  premises_zip = w.copy_zip
FROM insured_location_work AS w
WHERE p.id = w.policy_id
  AND p.tenant_id = (SELECT tenant_id FROM params)
  AND w.action = 'fill'
  AND NULLIF(regexp_replace(coalesce(p.premises_address, ''), '[[:space:]]+', '', 'g'), '') IS NULL
  AND NULLIF(regexp_replace(coalesce(p.premises_city, ''), '[[:space:]]+', '', 'g'), '') IS NULL
  AND NULLIF(regexp_replace(coalesce(p.premises_state, ''), '[[:space:]]+', '', 'g'), '') IS NULL

-- Preview (READ ONLY): Marketplace / Medicare policies whose insured location
-- WILL be filled from the linked contact.
--
-- Does not write. Not a drizzle migration — db:migrate will not run this.
-- Default tenant is the shared params CTE (TENANT_ID 11111111-1111-4111-8111-111111111111).
-- If the live tenant id differs, change that uuid in the shared block of all three files.
--
--   psql "$DATABASE_URL" -f scripts/preview-marketplace-medicare-insured-location.sql
--
-- policies_to_fill is the count, repeated on each listed row. When nothing matches,
-- the query returns one row with policies_to_fill = 0.
-- copy_state is the USPS code that will be written to policies.premises_state.
-- Filled and flagged counts are only known after this runs on the live database.
...
-- SHARED-END
SELECT
  count(*) OVER ()::bigint AS policies_to_fill,
  policy_id,
  policy_number,
  contact_name,
  product_family,
...
SELECT
  ...
  NULL::text
WHERE NOT EXISTS (
  SELECT 1 FROM insured_location_work WHERE action = 'fill'
)

...
async function main() {
  ...
  const sheets = await db.execute(sql`
    select id, line, values, updated_at from quote_sheets where deal_id = ${HO3} order by line
  `);

  let riskArr: any[] = [];
  try {
    const risks = await db.execute(sql`select * from risks where deal_id = ${HO3}`);
  ...
  const lines: string[] = [];
  lines.push(`# Gloria Martinez — HO3 Master Quote Sheet Export`);
  lines.push(``);
  lines.push(`- **Deal ID:** \`${HO3}\``);
  ...
  lines.push(``);
  lines.push(`### Notes`);
  lines.push(`- Dec extracted mailing/property address may differ from agent-confirmed Adriatico LN / Miami Lakes mailing — see sheet \`records_check\` / address fields.`);
  lines.push(`- Co-applicant block is entirely empty (expected if single named insured).`);
  lines.push(``);

  const out = "/tmp/gloria-ho3-sheet.md";
